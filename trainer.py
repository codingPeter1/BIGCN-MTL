...
class Trainer(object):
    ...
    def metric_dict2str(self,metric_dict):
    ...
    def train_model(self):
        ...
        for epoch in range(self.epochs):
            ...
            test_metric_dict, validate_metric_dict, run_time = self._train_one_epoch(train_dataset_loader, epoch)
            total_time = total_time + run_time
            result = validate_metric_dict['recall@20']
            # early stop
        ...
        saved_emb = best_model.get_saved_layer_embs()
        saved_dict = collections.OrderedDict(
            {k: [copy.deepcopy(emb.detach()) for emb in v] for k, v in saved_emb.items()})
        logger.info(f"training end, best iteration %d, results: %s \n %s" %
                    (best_epoch + 1, best_dict.__str__(), self.metric_dict2str(best_dict)))

        logger.info(f"final test result is:  %s \n %s" % (final_test.__str__(), self.metric_dict2str(final_test)))
        logger.info(f"epoch is:  %s" % (epoch+1))
        logger.info(f"average time per epoch is:  %s" % (total_time/(epoch+1)))
    ...
    def _train_one_epoch(self, behavior_dataset_loader, epoch):
        ...
        return test_metric_dict, validate_metric_dict, run_time
    ...
    def save_model(self, model):
        ...
        torch.save(model.state_dict(), os.path.join(full_model_path,'his_weight_'+str(self.his_weight)+'kd_'+str(self.kd_weight)+'tao'+str(self.tao)+'_lr_'+str(self.learning_rate)+'_'+'reg_'+str(self.reg_weight)+'_'+self.model_name + self.TIME + '.pth'))
